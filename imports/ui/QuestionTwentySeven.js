...
const AppQ = (values, handleChange, handleSubmit) => {
  return (<div className='wrapper'>
    <div className='questionList'>
      <p className='questionList__message'>27. Overall, do you think that Severe Weather Warnings/Watches are:</p>
    </div>

    <Form>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_27_answer' value='not issued enough'/>
          not issued enough
        </label>
      </div>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_27_answer' value='issued about as often as needed'/>
          issued about as often as needed
        </label>
      </div>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_27_answer' value='issued too often'/>
          issued too often
        </label>
      </div>
...
const FormikApp = withFormik({
  ...
  handleSubmit(values) {
    ...
    if (values.q_27_answer) {
      browserHistory.push("/q28");
    }
    ...
    Survey.update({
      ...
      $set: {
        Question_27: values.q_27_answer
      }
