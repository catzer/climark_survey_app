...
export default class QuestionTwentyThree extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      survey: []
...
const AppQ = (values, handleChange, handleSubmit) => {
  return (<div className='wrapper'>
    <div className='questionList'>
      <p className='questionList__message'>23. Which prompts you to take a more serious approach to the conditions of severe weather?</p>
    </div>

    <Form>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_23_answer' value='Severe Weather Watch'/>
          Severe Weather Watch
        </label>
      </div>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_23_answer' value='Severe Weather Warning'/>
...
const FormikApp = withFormik({
  ...
  handleSubmit(values) {
    ...
    Survey.update({
      ...
      $set: {
        Question_23: values.q_23_answer
      }

    });
    if (values.q_23_answer) {
      browserHistory.push("/q24");
    }
