...
export default class QuestionThirtyNine extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      survey: []
  ...
  return (<div className='wrapper'>
    <div className='questionList'>
      <p className='questionList__message'>
        39. Before the severe weather warning, I knew weather conditions were favorable for severe weather.</p>
    </div>

    <Form>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_39_answer' value='Strongly Agree'/>
          Strongly Agree
        </label>
      </div>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_39_answer' value='Agree'/>
          Agree
        </label>
      </div>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_39_answer' value=' Disagree'/>
          Disagree
        </label>
      </div>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_39_answer' value='Strongly Disagree'/>
          Strongly Disagree
        </label>
      </div>
...
const FormikApp = withFormik({
  ...
  handleSubmit(values) {
    ...
    Survey.update({
      ...
      $set: {
        Question_39: values.q_39_answer
      }

    });

    if (values.q_39_answer) {
      browserHistory.push("/q40");
    }
