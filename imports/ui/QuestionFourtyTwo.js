...
export default class QuestionFourtyTwo extends React.Component {
  constructor(props) {
  super(props);
  this.state = {
    survey: []
...
const AppQ = (values, handleChange, handleSubmit) => {
  return (<div className='wrapper'>
    <div className='questionList'>
      <p className='questionList__message'>42. There is limited weather information in my local language.</p>
    </div>

    <Form>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_42_answer' value='Strongly Agree'/>
          Strongly Agree
        </label>
      </div>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_42_answer' value='Agree'/>
          Agree
        </label>
      </div>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_42_answer' value=' Disagree'/>
          Disagree
        </label>
      </div>
      <div className='question__spacing'>
        <label >
          <Field type='radio' name='q_42_answer' value='Strongly Disagree'/>
          Strongly Disagree
        </label>
      </div>
...
const FormikApp = withFormik({
  ...
  handleSubmit(values) {
    ...
    Survey.update({
      ...
      $set: {
        Question_42: values.q_42_answer
      }

    });
    if (values.q_42_answer) {
      browserHistory.push("/q43");
    }
